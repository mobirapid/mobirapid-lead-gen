(function () {
  ...
  on('payuSaveBtn', 'click', () => {
    saveSettings(collectSettings(['payu_enabled', 'payu_mode', 'payu_merchant_key', 'payu_salt', 'reserve_thankyou_text', 'reserve_button_enabled', 'reserve_flat_amount', 'booking_percent', 'booking_min_amount', 'reserve_payment_link']), $('payuSaved'));
  });
  ...
  function openModel(id) {
    ...
    renderCondPriceRows();
    ['name', 'slug', 'price', 'mrp', 'best_for', 'price_note', 'specs', 'description', 'badge', 'condition_grade', 'warranty', 'cpu', 'gpu', 'memory', 'storage', 'display', 'software', 'battery_health', 'colour', 'image', 'sort_order'].forEach((f) => { if ($('m-' + f)) $('m-' + f).value = m ? (m[f] ?? '') : (f === 'sort_order' ? models.length + 1 : ''); });
